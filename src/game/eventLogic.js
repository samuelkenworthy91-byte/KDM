...
import { generalFightingArts } from '../data/fightingArts.js';
...
function applyEffects(effects, state, context) {
  const next = {
    ...
    runSurvivor: {
      ...
      deckAdditions: [...(state.runSurvivor.deckAdditions || [])],
  ...
  if (effects.gainSurvival) {
    next.runSurvivor.survival = (next.runSurvivor.survival || 0) + effects.gainSurvival;
    next.appliedEffects.push(`+${effects.gainSurvival} Survival`);
  }
  if (effects.addCardToDeck) {
    next.runSurvivor.deckAdditions.push(effects.addCardToDeck);
    next.appliedEffects.push(`Card added: ${effects.addCardToDeck}`);
  }
  if (effects.addPanic) {
    next.runSurvivor.deckAdditions.push(...Array(effects.addPanic).fill('panic'));
  ...
  if (effects.gainTrait) {
    ...
    if (effects.gainTrait.type === 'monsterBaneCurrent') {
      if (!next.runSurvivor.fightingArts.includes(trait)) next.runSurvivor.fightingArts.push(trait);
  ...
  if (effects.gainScar) {
    if (!next.runSurvivor.scars.includes(effects.gainScar)) {
      next.runSurvivor.scars.push(effects.gainScar);
    }
    next.appliedEffects.push(`Scar gained: ${effects.gainScar}`);
  }
  ...
  if (effects.removePanic) {
    const index = next.runSurvivor.deckAdditions.indexOf('panic');
    if (index >= 0) {
      next.runSurvivor.deckAdditions.splice(index, 1);
